...
Aislamiento 2026-09-21: pytest exige `TEST_DATABASE_URL` (`sin_linea_test`); aborta si falta o si es la base de la app; el `TRUNCATE` comprueba `current_database()` antes.

## Track C — revisión prueba real post-fixes — 2026-09-22

No merge. Conservación de versiones C11 del export se mantiene. Defectos evidencia↔claims↔audit corregidos en código; snapshots históricos y versiones publicadas no se reescribieron.

Congelamiento verificado: `AUTO_PUBLISH=false` en api y worker; Beat Exited (137); `app_settings.auto_poll_enabled=false`. Corpus `sin_linea` intacto; backup `.editorial-evals/backups/sin_linea-prueba-real-post-fixes-20260922T0038Z.dump`. Pytest solo contra `sin_linea_test`.

En código: excerpt `SUPPORTS` exige equivalencia de la proposición (Tablada «durante el tiroteo» → QUALIFIES); Flipr republicado colapsa a Infobae; `cited:` deja de ser primaria documental (t.co no cuenta); `expected_central` incluye el núcleo factual del titular; Audit descarta findings del modelo con claim de otro hecho y no elige un contrato de embargo incompatibles. Tests: `test_track_c_real_regressions.py` + módulos afectados (fakes). No hay lote RSS ni publicación.

Recovery dirigida 2026-09-22 (sin commit, API/worker reconstruidos, `AUTO_PUBLISH=false`, Beat ausente): backup `.editorial-evals/backups/sin_linea-two-case-precheck-20260922T0321Z.dump`. La Tablada `58759b17` V3 candidata (`confirmation_dropped`): claim `37f7556a` pasa de Confirmado/`SUPPORTED` a `SINGLE_SOURCE` / etiqueta **Respaldo limitado**; el cuerpo atribuye el disparo único a una sola línea informativa. Live sigue en V2. Audit V3 `structural_block` (C5 atribución/categórico en bajada, no el vínculo del tiroteo). Esteche `bb76b7a6` V3 (`coverage_gap_now_open`): `coverage_gap` live, `central_uncovered` HIGH, Audit `passed=false`. Recuperarla exige incorporar y evaluar la afirmación central del titular; no alcanza re-verificar claims secundarios. Live sigue en V2. Formosa (`a614745e` vs `45d3c314`) queda fuera.
